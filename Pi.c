...
int main() {
    ...
    srand(time(NULL)); //"srand" immer andere Zufallszahlen   "time(Null)" Zeit das immer andere Zufallszahlen sind
    ...
    for (long long int i = 0; i < num_points; i++) {  // Wiederholt Zufallspunktgeneration 1000000 mal
        ...
        if (x * x + y * y <= 1.0) {   //Satz des Pythagoras "<=1" ist in Kreis r=1
            ...
            inside_circle++;      //Pr¸fen, ob der Punkt im Viertelkreis liegt wenn ja +1
